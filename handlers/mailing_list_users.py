...
mailing_user_menu = types.InlineKeyboardMarkup(inline_keyboard=mailing_user_menu)
# back_kb = types.InlineKeyboardMarkup(inline_keyboard=button_back_to_admin)
...
@router.callback_query(F.data == 'user_news', IsAdmin())
async def user_news_get(query: types.CallbackQuery):
     await query.message.edit_text('🚹 Рассылка для юзеров', reply_markup=mailing_user_menu)

##  Функция рассылки пользователям
@router.callback_query(F.data == 'create_waiting_user_first', IsAdmin())
async def user_news_get(query: types.CallbackQuery, state: FSMContext, bot: Bot):
    await query.message.edit_text('👀 Введите пост для рассылки пользователям 🚹\n\n' \
                                  'Или нажмите кнопку выхода в главное меню',
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
        [types.InlineKeyboardButton(text="↩️ Отмена", callback_data="news")]]))
    
    await state.set_state(ScheduleStates.waiting_for_user_news_first)

@router.message(ScheduleStates.waiting_for_user_news_first)
async def schedule_message(message: types.Message, state: FSMContext):
    await state.set_state(ScheduleStates.waiting_date_for_user_first)
...
@router.message(ScheduleStates.waiting_date_for_user_first)
async def process_schedule_time(message: types.Message, state: FSMContext):
    try:
        ...
        await message.answer(
            ...
            reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
                [types.InlineKeyboardButton(text="➕ Добавить еще", callback_data="create_waiting_user_first")],
...
@router.callback_query(F.data == "list_scheduled_users", IsAdmin())
async def list_scheduled_messages(query: types.CallbackQuery):
    ...
    text = "📅 <b>Ваши отложенные сообщения для юзеров🚹:</b>\n\n"
    for msg in messages:
    ...
    await query.message.edit_text(
        ...
        reply_markup=types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="❌ Удалить", callback_data="delete_scheduled_menu_users")],
            [types.InlineKeyboardButton(text="🔙 Назад", callback_data="news")]
        ]
        )
    )
...
@router.callback_query(F.data == "delete_scheduled_menu_users", IsAdmin())
async def delete_scheduled_menu_users(query: types.CallbackQuery):
    messages = scheduled_messages.get(query.from_user.id, [])
    ...
    for msg in messages:
        buttons.append([
            types.InlineKeyboardButton(
                ...
                callback_data=f"delete_scheduled_users_{msg['id']}"
            )
        ])
    ...
    buttons.append([
        types.InlineKeyboardButton(
            text="🔙 Назад",
            callback_data="list_scheduled_users"
        )
    ])  
...
@router.callback_query(F.data.startswith("delete_scheduled_users_"), IsAdmin())
...
@router.callback_query(F.data == 'send_users_now', IsAdmin())
async def send_message_immediately(query: types.CallbackQuery, state: FSMContext):
    await state.set_state(AdminState.waiting_for_user_news)
    await query.message.edit_text('⚠️⚠️⚠️ Введите пост для немедленной рассылки юзерам 🚹:')
